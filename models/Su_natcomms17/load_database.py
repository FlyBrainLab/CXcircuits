...
cx_version = 'Su_natcomms17'
initial_drop = False
model_version = "Su_natcomms17"
...
neuropil_to_subregions = {'BU': ['R{}'.format(i) for i in range(1,4)],
                        'bu': ['L{}'.format(i) for i in range(1,4)],
...
family_to_file_list = {'BU-EB': data.files('ring.csv'),
    ...
    for post_rid in s:
        ...
        for pre_rid in b:
            ...
            if pre_rid != post_rid:
                ...
                if pre_family == 'BU-EB' and post_family == 'BU-EB':
                    pre_ind = [i for i, n in enumerate(pre_neuron_parse) if n['neuropil'] != 'EB'][0]
                    post_ind = [i for i, n in enumerate(post_neuron_parse) if n['neuropil'] != 'EB'][0]
                    if list(pre_neuron_parse[pre_ind]['regions'])[0][1] != '3' or list(post_neuron_parse[post_ind]['regions'])[0][1] != '3':
                        continue
                if pre_family == 'PB-EB-LAL' and post_family in ['BU-EB', 'PB-EB-NO', 'PB-EB-LAL']:
                    continue
                if pre_family == 'BU-EB' and post_family == 'PB-EB-LAL':
                    pre_ind = [i for i, n in enumerate(pre_neuron_parse) if n['neuropil'] != 'EB'][0]
                    if list(pre_neuron_parse[pre_ind]['regions'])[0][1] != '1': # R1/L1 are ring_PEG
                        continue
                if pre_family == 'PB-EB-NO' and post_family in ['BU-EB', 'PB-EB-LAL', 'PB-EB-NO']:
                    continue
                if pre_family == 'BU-EB' and post_family == 'PB-EB-NO':
                    pre_ind = [i for i, n in enumerate(pre_neuron_parse) if n['neuropil'] != 'EB'][0]
                    if list(pre_neuron_parse[pre_ind]['regions'])[0][1] != '2': # R2/L2 are ring_PEN
                        continue
                if pre_family == 'BU-EB' and post_family == 'EB-LAL-PB':
                    pre_ind = [i for i, n in enumerate(pre_neuron_parse) if n['neuropil'] != 'EB'][0]
                    if list(pre_neuron_parse[pre_ind]['regions'])[0][1] != '3': # R3/L3 are ring_EPG
                        continue
                if pre_family == 'EB-LAL-PB':
                    if post_family in ['PB-EB-NO', 'PB-EB-LAL', 'EB-LAL-PB'] and subregion.name.split('/')[0] == 'EB':
                        continue
                    if post_family in ['BU-EB']:
                        post_ind = [i for i, n in enumerate(post_neuron_parse) if n['neuropil'] != 'EB'][0]
                        if list(post_neuron_parse[post_ind]['regions'])[0][1] != '3':
                            continue
                name = pre_name+'->'+ \
                       post_name+'_in_'+ \
...
for neuropil in neuropils:
    ...
    for neuron in neurons:
        ...
        params = {'initV': -70.,
                  ...
                  'bias_current': 0.0}
        if family == 'BU-EB':
            if neuron.name.split('/')[-2][1] == '1':
                params['capacitance'] = 1e-4
            elif neuron.name.split('/')[-2][1] == '3':
                params['capacitance'] = 1e-2
...
for neuropil in neuropils:
    ...
    for synapse in synapses:
        ...
        q_post_neuron = q_synapse.gen_traversal_out(
                            ...
                            min_depth = 1)
        post_neuron_bio = q_post_neuron.get_as('obj')[0][0]
        ...
        if pre_family == 'BU-EB':
            if post_family == 'BU-EB':
                params = {'gmax': 1.6e-3*2/length, 'st': 5.0, 'reverse': -70.}
            elif post_family == 'EB-LAL-PB':
                params = {'gmax': 5e-3*2.4/length, 'st': 5.0, 'reverse': -70.}
            else: #post_family == 'PB-EB-NO' or post_family == 'PB-EB-LAL'
                params = {'gmax': 10e-3, 'st': 5.0, 'reverse': -70.}
            model_cls = graph.SynapseGABAs
        elif pre_family == 'EB-LAL-PB':
            if post_family == 'PB-EB-LAL':
                params = {'gmax': 5e-3*10, 'st': 100.0, 'Mg': 1.0, 'reverse': 0.}
            elif post_family == 'PB-EB-NO':
                params = {'gmax': 6e-3*10, 'st': 100.0, 'Mg': 1.0, 'reverse': 0.}
            else: #'BU-EB'
                params = {'gmax': 1e-3*10, 'st': 100., 'Mg': 1., 'reverse': 0.}
            model_cls = graph.SynapseNMDAs
        elif pre_family == 'PB-EB-LAL':
            # if post_family == 'BU-EB':
            #     params = {'gmax': 1.0e-3, 'st': 100., 'Mg': 20., 'reverse': 0.}
            if post_family == 'EB-LAL-PB':
                params = {'gmax': 4e-3*10., 'st': 100., 'Mg': 1., 'reverse': 0.}
            model_cls = graph.SynapseNMDAs
        elif pre_family == 'PB-EB-NO':
            # if post_family == 'BU-EB':
            #     params = {'gmax': 1.0e-3, 'st': 100., 'Mg': 20., 'reverse': 0.}
            if post_family == 'EB-LAL-PB':
                params = {'gmax': 6e-3*10, 'st': 100., 'Mg': 1., 'reverse': 0.}
            model_cls = graph.SynapseNMDAs
        ...
        if model_cls == graph.SynapseNMDAs:
            graph.SendsTo.create(post_neuron, exec_node, variable = 'V')
        if pre_neuron_LPU.name == neuropil_name:
            graph.SendsTo.create(pre_neuron, exec_node, variable = 'spike_state')
        else:
            ...
            graph.SendsTo.create(port, exec_node, variable = 'spike_state')
